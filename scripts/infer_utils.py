...
class Bbox:
    ...
    def __init__(self, outer_bbox = None):
        self.__outer_bbox = outer_bbox
        self.__interior_bboxes = []
        if outer_bbox:
            self.__interior_bboxes.append(outer_bbox)

    @property
    def outer_bbox(self):
        return self.__outer_bbox

    @outer_bbox.setter
    def outer_bbox(self, outer_bbox):
        self.__outer_bbox = outer_bbox

    @property
    def interior_bboxes(self):
        return self.__interior_bboxes

    def add_interior_bbox(self, bbox):
        self.__interior_bboxes.append(bbox)

    @property
    def min_x(self):
        return self.__outer_bbox[0]

    @property
    def min_y(self):
        return self.__outer_bbox[1]
    
    @property
    def max_x(self):
        return self.__outer_bbox[2]

    @property
    def max_y(self):
        return self.__outer_bbox[3]

    @min_x.setter
    def min_x(self, o):
        self.__outer_bbox[0] = o

    @min_y.setter
    def min_y(self, o):
        self.__outer_bbox[1] = o

    @max_x.setter
    def max_x(self, o):
        self.__outer_bbox[2] = o

    @max_y.setter
    def max_y(self, o):
        self.__outer_bbox[3] = o

    def scale(self, coef):
        self.__outer_bbox[0] *= coef[0]
        self.__outer_bbox[1] *= coef[1]
        self.__outer_bbox[2] *= coef[2]
        self.__outer_bbox[3] *= coef[3]

        if len(self.__interior_bboxes) > 1:
            for i in range(len(self.__interior_bboxes)):
                self.__interior_bboxes[i][0] *= coef[0]
                self.__interior_bboxes[i][1] *= coef[1]
                self.__interior_bboxes[i][2] *= coef[2]
                self.__interior_bboxes[i][3] *= coef[3]
    ...
    def sort_with_min_x(self):
        self.__interior_bboxes.sort(key=lambda x : x[0])


def get_row_bboxes_count(bboxes: List[Bbox]):
    cur = bboxes[0]
    count = 1
    min_y = cur.max_y
    ...
    for i in range(1, len(bboxes)):
        ...
        if cur.min_y > min_y:
            break
        count += 1
        min_y = min(min_y, cur.max_y)

    return count


def get_col_bboxes_count(bboxes: List[Bbox]):
    cur = bboxes[0]
    count = 1
    max_x = cur.max_x
    ...
    for i in range(1, len(bboxes)):
        cur = bboxes[i]
        if cur.min_x > np.mean(chosen_bboxes):
            break
        count += 1
        # max_x = min(max_x, cur[2])
        chosen_bboxes.append(cur.max_x)
...
def rescale_output_cls(bboxes: List[Bbox], current_shape, target_shape):
    # print(current_size, target_size)
    scale_x = target_shape[1]/current_shape[1]
    scale_y = target_shape[0]/current_shape[0]

    for i in range(len(bboxes)):
        bboxes[i].scale((scale_x, scale_y, scale_x, scale_y))


def read_pdf_windowed(fitz_doc_page, bbox: Bbox, embrace=(2, 7)):
    text = ""

    # print("reading", len(bbox.interior_bboxes))
    for box in bbox.interior_bboxes:
        read_text = fitz_doc_page.get_textbox(
            fitz.Rect(box[0] - embrace[0], box[1] - embrace[1], box[2] + embrace[0], box[3] + embrace[1])
        )
...
def draw_table_struct(image, bboxes, name, sample_name):
    ...
    bboxes.sort(key=lambda x : x.min_x)
    ...
    while total_count < len(bboxes):
        ...
        column_midpoint = round(np.array([(b.min_x, b.max_x) for b in bboxes[total_count:total_count+count]]).mean(axis=1).mean())
    ...
    bboxes.sort(key=lambda x : x.min_y)

    total_count = 0
    while total_count < len(bboxes):
        count = get_row_bboxes_count(bboxes[total_count:])

        for bbox in bboxes[total_count:total_count+count]:
            for box in bbox.interior_bboxes:
                cv2.rectangle(res_img, (round(box[0]), round(box[1])), (round(box[2]), round(box[3])), color_list[color_index], thickness=1)

            # cv2.rectangle(res_img, (round(bbox.min_x), round(bbox.min_y)), (round(bbox.max_x), round(bbox.max_y)), (0, 0, 0), thickness=1)
    ...
    # print("Saved table struct ", f"../res/preds/{name}/{sample_name}.png")
...
def to_excel_file(pdf_bboxes, fits_doc_page, doc_name, sample_name):
    ...
    pdf_bboxes.sort(key=lambda x : x.min_x)
    ...
    while total_count < len(pdf_bboxes):
        ...
        column_midpoint = np.array([(b.min_x, b.max_x) for b in pdf_bboxes[total_count:total_count+count]]).mean(axis=1).mean()
        column_midpoints.append(column_midpoint)
        total_count += count

    pdf_bboxes.sort(key=lambda x : x.min_y)
    ...
    while total_count < len(pdf_bboxes):
        ...
        sorted_row_bboxes = sorted(pdf_bboxes[total_count:total_count+count], key=lambda x : x.min_x)
        ...
        for box in sorted_row_bboxes:
            ...
            for col_ind, col_mid in enumerate(column_midpoints):
                ...
                distance_from_midpoint = abs((box.min_x + box.max_x) / 2 - col_mid)
        ...
        for box in sorted_row_bboxes:
            ...
            for col_ind, col_mid in enumerate(column_midpoints):
                if box.min_x <= col_mid and box.max_x >= col_mid:
                    intersected_column_indices.append(col_ind+1)
...
def detect_text_bboxes(input_image):
    ...
    for i, pred_box in enumerate(boxes):
        ...
        bbox1 = Bbox([b1[0], b1[1], b1[2], b1[3]])

        to_be_removed = []
        # x1, y1, x2, y2 = b1[0], b1[1], b1[2], b1[3]
        for j, bbox2 in enumerate(valid_boxes):

            if (
                do_intersect(
                    (bbox1.min_x, bbox1.min_y, bbox1.max_x, bbox1.max_y), 
                    (bbox2.min_x, bbox2.min_y, bbox2.max_x, bbox2.max_y), 
                margins=(3, 0)) 
                and 
                (
                    bbox1.min_y < (bbox2.max_y + bbox2.min_y) / 2 < bbox1.max_y 
                    or 
                    bbox2.min_y < (bbox1.max_y + bbox1.min_y) / 2 < bbox2.max_y
                )
            ):
                x1 = min(bbox2.min_x, bbox1.min_x)
                y1 = min(bbox2.min_y, bbox1.min_y)
                x2 = max(bbox2.max_x, bbox1.max_x)
                y2 = max(bbox2.max_y, bbox1.max_y)
                # valid_boxes.remove(pred_box)
                bbox1.outer_bbox = [x1, y1, x2, y2]
                # bbox1.add_interior_bbox([bbox2.min_x, bbox2.min_y, bbox2.max_x, bbox2.max_y])
                for inter_bbox in bbox2.interior_bboxes:
                    bbox1.add_interior_bbox(inter_bbox)
                to_be_removed.append(bbox2)
            # pass

        for bbox in to_be_removed:
            valid_boxes.remove(bbox)

        valid_boxes.append(bbox1)
...
def detect_table_bboxes(input_image, pdf_name=None, sample_name=None):
    ...
    process_output0 = postprocess_table_detector_output(raw_out, 2, 1000, max_seg_dist=20)
